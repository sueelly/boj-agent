...
## 13. Python config 모듈 테스트 (`src/core/config.py`)

> Issue #45 — Bash `config.sh`를 대체하는 Python config 모듈의 단위 테스트 전략.

### 13.1 테스트 위치

```
tests/unit/test_config.py    # Python config 모듈 단위 테스트
```

기존 `tests/unit/test_config.sh`(Bash)와 병행 유지. Python 전환 완료 후 Bash 테스트 제거.

### 13.2 격리 전략

config 모듈은 **파일시스템 + 환경변수 + git subprocess**에 의존한다.
모든 테스트는 다음을 격리해야 한다:
...
def config_env(tmp_path, monkeypatch):
    ...
    # BOJ_* 환경변수 초기화 (테스트 간 오염 방지)
    for key in ("BOJ_LANG", "BOJ_EDITOR", "BOJ_AGENT_CMD",
                "BOJ_USER", "BOJ_SOLUTION_ROOT", "BOJ_AGENT_ROOT"):
        monkeypatch.delenv(key, raising=False)

    return config_dir
```

- `monkeypatch.setenv/delenv`: 환경변수 격리 (테스트 후 자동 복원)
- `tmp_path`: 파일시스템 격리
- git 호출: `subprocess` mock 또는 `tmp_path`에 임시 git repo 생성

### 13.3 테스트 매트릭스

| 범주 | 테스트 | edge-cases 참조 |
|------|--------|----------------|
| **우선순위** | env > file > default 3단계 fallback | CF1, CF2, CF3 |
| **read/write** | set 후 get 라운드트립 | CF4 |
| **파일시스템** | 디렉터리 자동 생성, 권한 거부, 빈 파일, trailing whitespace | CF4, CF5, CF6, CF7 |
| **setup_done** | 파일 존재/부재 판단 | CF8, CF9 |
| **경로검증** | agent_root/solution_root 존재/부재 | CF10, CF11 |
| **언어검증** | 지원/미지원 언어, languages.json 파싱 실패 | CF12, CF13, CF14 |
| **agent 매핑** | 알려진 agent → 커맨드, 알 수 없는 agent | CF15, CF16 |
| **git 연동** | user.name/email 읽기, git 미설치 | CF17, CF18, CF19 |
| **check_config** | 포맷팅 출력 검증 | CF20 |
| **BOJ_CONFIG_DIR** | 환경변수로 디렉터리 오버라이드 | CF21 |

### 13.4 테스트 예시

```python
def test_get_returns_env_over_file_when_both_set(config_env, monkeypatch):
    """환경변수와 파일 모두 있을 때 환경변수 값을 반환한다."""
    # Arrange
...
def test_get_returns_default_when_no_env_no_file(config_env):
    """환경변수도 파일도 없으면 기본값을 반환한다."""
    result = config_get("prog_lang", "java")
    assert result == "java"


def test_set_creates_config_dir_when_missing(tmp_path, monkeypatch):
    """설정 디렉터리가 없으면 자동 생성 후 저장한다."""
...
def test_is_setup_done_returns_false_when_no_flag(config_env):
    """setup_done 파일이 없으면 False를 반환한다."""
    assert is_setup_done() is False


def test_validate_lang_rejects_unsupported(config_env):
    """미지원 언어에 대해 False를 반환한다."""
    assert validate_lang("fortran") is False


def test_validate_lang_accepts_supported(config_env):
    """지원 언어에 대해 True를 반환한다."""
...
git subprocess 호출은 `unittest.mock.patch`로 격리한다:

```python
from unittest.mock import patch

def test_get_git_user_name_when_configured():
    """git user.name이 설정되어 있으면 값을 반환한다."""
...
def test_get_git_user_name_when_not_configured():
    """git user.name이 미설정이면 빈 문자열을 반환한다."""
    with patch("subprocess.run") as mock_run:
...
## 16. client (`tests/unit/test_client.py`)

| # | 테스트 | 커버리지 |
|---|--------|----------|
| 1 | 세션 파일 있어도 Cookie 미전송 | CL1 |
| 2 | 세션 없으면 Cookie 미전송 | CL1 |
| 3 | 403 → exit(1) | CL3 |
| 4 | 404 → exit(1) | CL2 |
| 5 | BOJ_CLIENT_TEST_HTML bypass | CL5 |
| 6 | 로그인 성공 → 쿠키 반환 | - |
| 7 | 로그인 실패 → ValueError | - |

## 17. normalizer (`tests/unit/test_normalizer.py`)

| # | 테스트 | 커버리지 |
|---|--------|----------|
| 1 | fixture 스냅샷 비교 (4개) | NR1 |
| 2 | 제목/번호/링크/샘플/제한 포함 (4×5) | NR1 |
| 3 | 빈 samples | NR2 |
| 4 | 선택 필드 없음 | NR3 |
| 5 | HTML 엔티티 보존 | NR5 |
| 6 | 이미지 태그 보존 | NR4 |
...
| # | 테스트 | 대상 문제 |
|---|--------|-----------|
| 1 | problem.json 필수 필드 검증 | 1516, 16957, 10799, 10951 |
| 2 | README에 제목 포함 | 1516, 16957, 10799, 10951 |
| 3 | README에 문제 설명 포함 | 1516, 16957, 10799, 10951 |
| 4 | README에 예제 입출력 포함 | 1516, 16957, 10799, 10951 |
| 5 | 이미지 추출 (10799) | 10799 |
| 6 | 이미지 다운로드 + 저장 | 10799 |
| 7 | 이미지 매직바이트 검증 | 10799 |
| 8 | URL → 로컬 경로 치환 | 10799 |
