...
| `BOJ_CONFIG_DIR` | 설정 디렉터리를 임시 경로로 격리 (현재 config.sh 미지원 — Python 전환 시 구현 예정) |
...
def test_creates_readme_when_valid_problem(boj_env, fixture_path):
    ...
    # Act
    result = run_boj(env, "make", "99999", "99999")
...
| `commit` | 커밋 생성 | git 아님, 폴더 없음, email 미설정 | 변경사항 없음, 통계 조회 실패 | CT1-CT9 |
| `submit` | Submit.java 생성 + 컴파일 | 솔루션 없음, 미지원 언어 | Parse 없는 경우, 기존 Submit 존재 | SB1-SB10 |
| `open` | 에디터 호출 | 폴더 없음 | 에디터 미설정 | O1-O3 |
...
def test_make_creates_problem_json(boj_env, problem_num):
    ...
    result = run_boj(env, "make", problem_num, "99999")

    prob_dir = next(tmp_path.glob(f"{problem_num}*"), None)
    assert prob_dir is not None
    assert (prob_dir / "artifacts" / "problem.json").exists()  # 목표 구조 — 코드 전환 시 적용
...
def test_make_generates_signature_review(boj_env, fixture_path):
    ...
    review = prob_dir / "artifacts" / "signature_review.md"  # 목표 구조
...
# 파일 생성 검증
assert (prob_dir / "artifacts" / "problem.json").exists()  # 목표 구조
...
def test_commit_creates_git_commit(boj_env, fixture_path):
    ...
    result = run_boj(env, "commit", "99999", input_text="n\n")
    log = subprocess.run(
