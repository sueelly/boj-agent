...
### 비대화형 플래그
- `--root <경로>`: boj_solution_root 설정 (경로 검증)
...
|----------|-------------|------|
| `--root` | `boj_solution_root` | validate_path → config_set |
...
**플래그**: `--lang`, `--image-mode download|reference|skip`, `--no-open`, `--keep-artifacts`
...
[Step 2] Spec: README + problem.json → problem.spec.json (Agent)
    ├── 에이전트 있음:
    │   ├── prompts/boj-spec-generator.md 프롬프트 + reference/spec/ 참조
    │   └── artifacts/problem.spec.json 생성
    └── 에이전트 없음:
        └── Warning: spec 생성 스킵, Step 3에서 기존 방식 fallback
...
[Step 3] Solution + Parse 생성 (Agent)
    ...
    ├── 에이전트 있음 + spec 없음:
    │   ├── 기존 방식 (README + problem.json 기반)
    │   └── prompts/make-skeleton.md fallback 모드
    └── 에이전트 없음:
...
| 프롬프트 | 용도 |
|----------|------|
| `prompts/boj-spec-generator.md` | Step 2: spec 생성 (boj-spec-kit 기반) |
