...
4. setup_done 가드 (C1): setup/help/빈 입력 외 모든 명령어에서 setup_done 플래그 확인
   ├── 있음 → 계속
   └── 없음 → "설정이 완료되지 않았습니다" 안내 + boj setup 자동 실행
       └── setup 후에도 setup_done 없으면 Error + exit 1
5. 서브커맨드 디스패치: exec "$ROOT/src/commands/$SUBCOMMAND.sh" "$ROOT" "$PROBLEM_NUM" "${@:3}"
```
...
```
1. setup_done 가드 → 디스패처(src/boj)에서 공통 처리 (C1)

2. 에이전트 설정 확인
   └── 없음 → "에이전트가 설정되지 않았습니다" + exit 1
...
[Step 2] Open: 에디터 선오픈 (README 직후, spec 이전)
    ├── --no-open이 아니고 editor 설정된 경우 즉시 오픈
    ├── 사용자는 spec/skeleton 대기 중 문제 본문 확인 가능
    ├── --no-open 또는 editor 미설정이면 스킵
    └── spec 실패 시 -f 재시도 전 이미 README를 볼 수 있는 상태

[Step 3] Spec: README + problem.json → problem.spec.json (Agent)
...
[Step 4] Solution + Parse 생성 (Agent)
...
**파일**: `src/cli/boj_review.py` + `src/core/review.py`
**플래그**: `--lang`, `--root`, `--agent-root`

### 플로우

```
1. 문제 폴더 찾기 (find_problem_dir)
2. Solution 파일 탐색 (find_solution_file) — 없으면 Warning (리뷰는 계속)
3. 프롬프트 빌드 (build_review_prompt):
   ├── prompts/review.md 템플릿 읽기
   └── Solution 파일 내용 포함

4. 에이전트 있음:
   ├── run_review(problem_dir, agent_cmd, prompt)
   ├── stdout을 submit/REVIEW.md로 저장 (write_review_file)
   │   ├── submit/ 디렉터리 자동 생성
   │   └── stdout이 빈 경우 파일 미생성
   └── stderr로 "submit/REVIEW.md 저장 완료" 안내
5. 에이전트 없음:
   ├── "리뷰해줘" 클립보드 복사
   └── 에디터로 문제 폴더 열기
```

### 핵심 파일 구조 (core/review.py)

| 함수 | 역할 |
|------|------|
| `find_solution_file(problem_dir, lang)` | Solution 파일 탐색 (언어별 우선순위) |
| `build_review_prompt(problem_dir, template)` | 리뷰 프롬프트 조립 |
| `run_review(problem_dir, agent_cmd, prompt)` | 에이전트 subprocess 실행 |
| `write_review_file(problem_dir, content)` | submit/REVIEW.md 저장 |
| `clipboard_fallback(prompt)` | 클립보드 복사 (pbcopy/xclip) |
| `review(problem_num, ...)` | 메인 오케스트레이션 |
