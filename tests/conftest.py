...
마커:
    live: 실제 BOJ HTTP 연결이 필요한 테스트. --run-live로 활성화.
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="실제 BOJ 서버에 연결하는 라이브 테스트 실행",
    )
    parser.addoption(
        "--run-agent",
        action="store_true",
...
def pytest_configure(config):
    """커스텀 마커 등록."""
    config.addinivalue_line("markers", "live: 실제 BOJ HTTP 연결 필요 (--run-live)")
...
def pytest_collection_modifyitems(config, items):
    ...
    if not config.getoption("--run-live"):
        skip_live = pytest.mark.skip(reason="--run-live 옵션이 필요합니다")
        for item in items:
